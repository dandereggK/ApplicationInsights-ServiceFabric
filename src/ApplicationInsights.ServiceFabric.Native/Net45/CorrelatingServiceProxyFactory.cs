namespace Microsoft.ApplicationInsights.ServiceFabric.Remoting.Activities
{
    ...
    public class CorrelatingServiceProxyFactory : IServiceProxyFactory
    {
        ...
        /// </param>
        /// <returns>The proxy that implement the interface that is being remoted. The returned object does not implement <see cref="T:Microsoft.ServiceFabric.Services.Remoting.Client.IServiceProxy" /> interface.</returns>
        /// </summary>
            ...
            TargetReplicaSelector targetReplicaSelector = TargetReplicaSelector.Default, string listenerName = null)
        {
            ...
            TServiceInterface proxy = this.serviceProxyFactory.CreateNonIServiceProxy<TServiceInterface>(serviceUri,partitionKey, targetReplicaSelector, listenerName);
            this.methodNameProvider.AddMethodsForProxyOrService(proxy.GetType().GetInterfaces(),typeof(Object));
            return proxy;
        }
